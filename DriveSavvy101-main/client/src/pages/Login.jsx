...
const Login = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      if (state === "Sign Up") {
        const { data } = await axios.post(backendUrl + '/api/auth/register', { name, phone, role, email, password });
        if (data.success) {
          setIsLoggedin(true);
          getUserData();
          navigate('/');
          ...
          {state === 'Sign Up' && (
            <>
              ...
              <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
                ...
                <input onChange={e => setPhone(e.target.value)} value={phone} className='bg-transparent outline-none' type="text" placeholder='Phone Number' required />
              </div>
              <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
                <img src={assets.car_icon} alt="" />
                <input onChange={e => setRole(e.target.value)} value={role} className='bg-transparent outline-none' type="text" placeholder='Your Role (Driver or Owner)' required />
              </div>
            </>
          )}
